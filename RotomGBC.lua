return function(_p)
	...
	_p.DataManager:preload(8695783637) -- font
	...
	_p.DataManager:preload(8691805402,8678663334,8678663099,8678663185)  -- all decals
	...
	local SHEET_ID = 'rbxassetid://8539475889' -- <-- Not really needed since we did most of it without one
	...
	local function write(text, parent, px, py, timedFill, rightJustify)
		...
		local newSize = UDim2.new(0.0, 0, 1/psy, 0) 
		newSize = UDim2.new(0, 0, newSize.Y.Scale * 1, 0)
		...
		for _, v in pairs(frame:GetChildren()) do
			pcall(function()v.ResampleMode=Enum.ResamplerMode.Pixelated end)
		end
		return frame
	end
	...
	create 'ImageLabel' { -- top-left
		...
		Size = UDim2.new(176/352, 0, 176/352, 0),
	...
	create 'ImageLabel' { -- top-right
		...
		Size = UDim2.new(176/352, 0, 176/352, 0),
		Position = UDim2.new(176/352, 0, 0.0, 0),
		ZIndex = 4, Parent = worldmap,
		ResampleMode = Enum.ResamplerMode.Pixelated
	}
	create 'ImageLabel' { -- bottom-left
		...
		Size = UDim2.new(176/352, 0, 176/352, 0),
		Position = UDim2.new(0.0, 0, 176/352, 0),
		ZIndex = 4, Parent = worldmap,
		ResampleMode = Enum.ResamplerMode.Pixelated
	}
	create 'ImageLabel' { -- bottom-right
		...
		Size = UDim2.new(176/352, 0, 176/352, 0),
		Position = UDim2.new(176/352, 0, 176/352, 0),
		ZIndex = 4, Parent = worldmap,
		ResampleMode = Enum.ResamplerMode.Pixelated
	}
						...
						for i = #extraMapBuffers.top+1, requiredTopBuffers do
							local b = create 'Frame' {
								...
								create 'ImageLabel' { -- top-left
									...
									--ImageRectSize = Vector2.new(176, 22),
									Size = UDim2.new(176/352, 0, 1, 0),
									ZIndex = 3,
									ResampleMode = Enum.ResamplerMode.Pixelated
								},
								create 'ImageLabel' { -- top-right
						...
						for i = #extraMapBuffers.left+1, requiredLeftBuffers do
							local b = create 'Frame' {
								...
								create 'ImageLabel' {
									...
									Size = UDim2.new(1.0, 0, 176/352, 0),
									ZIndex = 3,
									ResampleMode = Enum.ResamplerMode.Pixelated
								},
								create 'ImageLabel' {
						...
						for i = #extraMapBuffers.bottom+1, requiredBottomBuffers do
							local b = create 'Frame' {
								...
								create 'ImageLabel' {
									...
									Size = UDim2.new(176/352, 0, 1.0, 0),
									ZIndex = 3,
									ResampleMode = Enum.ResamplerMode.Pixelated
								},
								create 'ImageLabel' {
	...
	local partyIndicator = create 'ImageLabel' {
		BackgroundTransparency = 1.0,
		Image = 'rbxassetid://8691805402', -- --> its originally suppose to seperate the empty party and your primary pokemon but that is just stupid  if you want to use original --> rbxassetid://8691777963
		...
		create 'ImageLabel' { -- ball
			BackgroundTransparency = 1.0,
			Image = '', -- rbxassetid://8691753600  <-- the leading ball
...
--[[
	local partyIndicator = create 'ImageLabel' {
		...
		create 'ImageLabel' { -- ball
			BackgroundTransparency = 1.0,
			Image = 'rbxassetid://8691237143',
			ImageRectSize = Vector2.new(192, 32),
			--ImageRectOffset = Vector2.new(0, 3*8*4),
			Size = UDim2.new(1/10, 0, 1/2, 0),
			Position = UDim2.new(2/10, 0, 0.0, 0),
			ZIndex = 4,
	...
	battle = function()
		...
		_p.MusicManager:stackMusic(13488342842, 'RotomBattle', .5)
	...
	function event:activate()
		...
		_p.MusicManager:stackMusic(13488362368, 'LavenderTown', .3)
	end
