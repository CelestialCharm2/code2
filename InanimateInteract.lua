return function(_p)
	...
	local function TweenFunction(mainpart,timer,easingstyle,easingdirection,repeatable, goals)
		local TweenService = game:GetService("TweenService")
		local Info = TweenInfo.new(timer,easingstyle,easingdirection,0,repeatable,0)
		local tween = TweenService:Create(mainpart,Info,goals)
		tween:Play()
	end
	return {
		...
		Rayquaza = function(model)

			if not _p.MasterControl.WalkEnabled then return end
			_p.MasterControl.WalkEnabled = false
			_p.MasterControl:Stop()
			local chat = _p.NPCChat
			spawn(function() chat:say('QUAZAAAA 2.0!')end)
			spawn(function() _p.Battle._SpriteClass:playCry(nil, {id = 000, startTime = 7.41, duration = 1.72}) end)
			wait(1.7)
			chat:manualAdvance()
			wait(.5)
			delay(3, function() model:Remove() end)
			_p.Battle:doWildBattle(_p.DataManager.currentChunk.regionData.Rayquaza, {musicId = 13059437179})
			_p.NPCChat:say('Rayquaza has soared to the moon.')
			_p.MasterControl.WalkEnabled = true
		end,
		Kyurem = function(model)
			if not _p.MasterControl.WalkEnabled then return end
			_p.MasterControl.WalkEnabled = false
			_p.MasterControl:Stop()
			local chat = _p.NPCChat
			spawn(function() chat:say('YOU WILL NOT CATCH ME!')end)
			spawn(function() _p.Battle._SpriteClass:playCry(nil, {id = 000, startTime = 7.41, duration = 1.72}) end)
			wait(1.7)
			chat:manualAdvance()
			wait(.5)
			delay(3, function() model:Remove() end)
			_p.Battle:doWildBattle(_p.DataManager.currentChunk.regionData.Kyurem, {musicId = 13059437179})
			spawn(function() _p.PlayerData:completeEvent('KyuremBattle2') end)
			_p.NPCChat:say('Kyurem can now be found roaming in the wild.')
			_p.MasterControl.WalkEnabled = true
		end,
		...
		RayquazaWall = function(model)
			if not _p.MasterControl.WalkEnabled then return end
			_p.MasterControl.WalkEnabled = false
			_p.MasterControl:Stop()
			local canOpen, done
			Utilities.fastSpawn(function()
				canOpen = _p.Network:get('PDS', 'hasKandG')
				done = true
			end)
			spawn(function() _p.MasterControl:LookAt(model.Main.Position) end)
			_p.NPCChat:say('When fire and water unite.',
				'The Mystery will unreveal......'
			)
			while not done do wait() end
			if canOpen then
				local DoorMain = model.Main
				...
				Utilities.Tween(3, "easeInSine", function(z)
					Utilities.MoveModel(DoorMain, DoorCFrame + Vector3.new(629.766, 126.522, 496.92))
				end)
				pcall(function() _p.PlayerData:completeEvent('hasKandG') end)
				_p.NPCChat:say('The Mysterious door opened, revealing a cave.')
			end
			_p.MasterControl.WalkEnabled = true
		end,
		...
		Yveltal = function(model)
		--	if not _p.PlayerData.completedEvents.YveltalCutscene then return end
			if not _p.MasterControl.WalkEnabled then return end
			local chunk = _p.DataManager.currentChunk
			if not chunk or chunk.id ~= 'chunkYveltal' then return end
			local enc
			pcall(function() enc = chunk.regionData.Yveltal end)
			if not enc then return end
			pcall(function() model['#InanimateInteract']:Remove() end)
			_p.MasterControl.WalkEnabled = false
			_p.MasterControl:Stop()
		...
		--	_p.Battle._SpriteClass:playCry(nil, {id = 10840591115, startTime = 2.93, duration = 1.66})
			delay(3, function() model:Remove() end)
			_p.Battle:doWildBattle(enc, {musicId = 17099979720})
			pcall(function() _p.PlayerData:completeEvent('Yveltal') end)
			_p.MasterControl.WalkEnabled = true
			_p.MasterControl:Stop()
			_p.NPCChat:say('Yveltal can now be found causing havok in the wild.')
		end,
		Xerneas = function(model)
			--	if not _p.PlayerData.completedEvents.YveltalCutscene then return end
			if not _p.MasterControl.WalkEnabled then return end
			local chunk = _p.DataManager.currentChunk
			if not chunk or chunk.id ~= 'chunk12' then return end
			local enc
			pcall(function() enc = chunk.regionData.Xerneas end)
			if not enc then return end
			pcall(function() model['#InanimateInteract']:Remove() end)
			_p.MasterControl.WalkEnabled = false
			_p.MasterControl:Stop()
			...
			delay(3, function() model:Remove() end)
			_p.Battle:doWildBattle(enc, {musicId = 17099979720})
			pcall(function() _p.PlayerData:completeEvent('Xerneas') end)
			_p.MasterControl.WalkEnabled = true
			_p.MasterControl:Stop()
			_p.NPCChat:say('Xerneas can now be found roaming in the wild.')
		end,
		...
		jackolantern = function(model)
			...
			onObtainItemSound()
			pcall(function() model.berries:Remove() end)
		...
		Mewtwo = function(model)
			if not _p.MasterControl.WalkEnabled then return end
			pcall(function() _p.PlayerData:completeEvent('Mewtwo') end)
			_p.MasterControl.WalkEnabled = false
			_p.MasterControl:Stop()
			local chat = _p.NPCChat
			pcall(function() model['#InanimateInteract']:Remove() end)
			spawn(function() _p.MasterControl:LookAt(model.Main.Position) end)
			spawn(function() chat:say(model.Main, 'RAHHH') end)
			wait(1.7)
			chat:manualAdvance()
			wait(.5)
			delay(3, function() model:Remove() end)
			_p.Battle:doWildBattle(_p.DataManager.currentChunk.regionData.Mewtwo, {musicId = 13059437179})
			_p.NPCChat:say('Mewtwo can now be found roaming in the wild')
			_p.MasterControl.WalkEnabled = true
		end,
		...
		monitorchange = function(model)

			local camera = workspace.CurrentCamera
			local chunk = _p.DataManager.currentChunk
			local case = chunk.map.Case
			local scene = chunk.map.Scene
			local chat = _p.NPCChat
			local CameraShaker = _p.CameraShaker
			local m2 = chunk.map.Mew2

			if _p.PlayerData.completedEvents.m2 then
				chunk.map.monitorCHANGE.screen.Color = Color3.new(0.0352941, 0.537255, 0.811765)
				chunk.map.monitorCHANGE.Main:Remove()
			else
				camera.CameraType = Enum.CameraType.Scriptable
				spawn(function() _p.Menu:disable() end)
				_p.MasterControl.WalkEnabled = false
				_p.MasterControl:Stop()
				chunk.map.monitorCHANGE.Main:Remove()
				TweenFunction(chunk.map.monitorCHANGE.screen, 2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out, false, {Color = Color3.new(0.0352941, 0.537255, 0.811765)})
				chat:say('System Accessed.',
					'Welcome, Dr. Fuji.',
					'Please select an opti-')
				...
				case:Remove()
				for i,v in pairs(m2.AnimationController:GetPlayingAnimationTracks()) do
					v:Stop()
				end
				local anim = m2.AnimationController:LoadAnimation(create 'Animation' { AnimationId = 'rbxassetid://'.._p.animationId.m2idle})
				anim:Play()
				m2.RootPart.CFrame = scene.m2land.CFrame
				m2.Main.CFrame = scene.m2land.CFrame

				task.wait(0.6)
				camShake:Stop()

				TweenFunction(camera,1,Enum.EasingStyle.Quad,Enum.EasingDirection.Out,false,{CFrame = scene.Cam2.CFrame, FieldOfView = 35})
				wait(3)
				spawn(function() _p.Menu:enable() end)
				spawn(function() TweenFunction(camera,1,Enum.EasingStyle.Quad,Enum.EasingDirection.Out,false,{FieldOfView = 70}) end)
		...
		mew2 = function(model)
			...
			if _p.PlayerData.completedEvents.m2 then
				...
				_p.MasterControl:Stop()
				_p.MasterControl.WalkEnabled = false
				local anim = m2.AnimationController:LoadAnimation(create 'Animation' { AnimationId = 'rbxassetid://'.._p.animationId.m2cutscene})
				for i,v in pairs(m2.AnimationController:GetPlayingAnimationTracks()) do
					v:Stop()
				end
				m2.Main:Remove()
				TweenFunction(camera,0,Enum.EasingStyle.Quad,Enum.EasingDirection.Out,false,{CFrame = scene.CamLook.CFrame, FieldOfView = 60})
				anim:Play()
				wait(1)
				TweenFunction(camera,9,Enum.EasingStyle.Quad,Enum.EasingDirection.Out,false,{CFrame = scene.Cam3.CFrame, FieldOfView = 60})
				wait(9)
				_p.Battle:doWildBattle(chunk.regionData.Mewtwo, {battleSceneType = 'Mewtwo'})
				m2:Remove()
				-- complete event
				spawn(function() _p.Menu:enable() end)
				_p.MasterControl.WalkEnabled = true
