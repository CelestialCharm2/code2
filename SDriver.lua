...
_f.ExternalDatastore = function(name, specificValue, splitToTable)
	local dataURLs = {
		Dynamax = 'https://pastebin.com/raw/srYg5ns8',
		FFlag = 'https://pastebin.com/raw/9X4m5AzP',
	...
	if specificValue then
		if type(specificValue) == 'table' then
			...
			for _, neededValue in pairs(specificValue) do
				returnTbl[neededValue] = string.split(_f.savedData[name][neededValue], dividerKey)
			end
			return returnTbl
...
for _, name in pairs({'Network', 'Context', 'DataService', 'Elo', 'BattleEngine', 'Backend'}) do -- BattleEngine just has to be installed before DataPersistence, and Elo before BattleEngine
