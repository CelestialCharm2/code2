...
Add AlliesAndSelf()
	...
	['supremeoverlord'] = {
		...
		onEnd = function(pokemon)
			if pokemon.fallen then
				self:add('-end', pokemon, pokemon.fallen .. pokemon.fallen, '[silent]')
				print('Supreme Overlord onEnd: fallen = ' .. pokemon.fallen)
			end
		end,
		...
		onBasePower = function(basePower, attacker, defender, move)
			if attacker.fallen then
				...
				print('Supreme Overlord onBasePower: fallen = ' .. attacker.fallen)
				print('Supreme Overlord boost: ' .. powMod[attacker.fallen] .. '/4096')
	...
	['opportunist'] = {
		onStart = function(pokemon)
			local fallen = 0
			if pokemon.side.totalFainted > 0 then
				self:add('-activate', pokemon, 'ability: Opportunist')
				fallen = math.min(pokemon.side.totalFainted, 5)
				self:add('-start', pokemon, fallen .. fallen, '[silent]')
				pokemon.fallen = fallen
			end
		...
		onModifySpe = function(spe, pokemon)
			if pokemon.fallen then
				local powMod = {4506, 4915, 5325, 5734, 6144}
				return self:chainModify({powMod[pokemon.fallen], 4096})
			end
		end,
		id = "opportunist",
		name = "Opportunist",
