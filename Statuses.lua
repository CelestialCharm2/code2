...
--			require(game.ServerStorage.Utilities).print_r(effect)
	...
	dynamax = {
		...
		onStart = function(pokemon)
			...
			if (pokemon.gigantamax) then
				local template = self:getTemplate('Charizard-gmax')
				local e = self:getTemplate('Charizard')
				self:add('detailschange', pokemon, pokemon.details, '[chargmax]', '[icon] '..(e.icon or 0))
				...
				self:setupDataForTransferToPlayers('Sprite', shinyPrefix..'_FRONT/Charizard-gmax')
				self:setupDataForTransferToPlayers('Sprite', shinyPrefix..'_BACK/Charizard-gmax')
			end
		...
		onBattleEnd = function(pokemon)
			if (pokemon.gigantamax) then self:add('-formechange', pokemon, pokemon.template.species) end
			pokemon.currentDyna = false
	...
	maxraid = {
		...
		onStart = function(pokemon)
			...
			if (pokemon.template.species == 'Shedinja') then return end
			self:add('-start', pokemon, 'maxRaid')
