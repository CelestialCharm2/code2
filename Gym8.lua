return function(_p)
	...
	local function tweenModel(model, cf, info)
		...
		local oriCF = model:GetPrimaryPartCFrame()
		local oriGoToCF = cf
		...
		tween.Completed:Connect(function()
			CFrameValue:Remove()
			model:SetPrimaryPartCFrame(oriGoToCF)
		end)
	end
		...
		if tostring(flipDir.Value) == '0, 0, 1' then
			if tostring(otherFlip.Value) == '0, 1, 0' then
				Utilities.Tween(1, 'linear', function(a)
					...
					map.Room6.CaptainNPC.HumanoidRootPart.CFrame = map.Room6.capnTP.CFrame
	...
	function gym8:foldRoom(timeToMove, pedestal)
		...
		if canFinish then
			...
			char:SetPrimaryPartCFrame(savedPlayerPos)
			savedPlayerPos = nil
			humanoidRoot.Anchored = false
			gym8:updateRoom()
	...
	function gym8:unfoldRoom(timeToMove, pedestal)
		if pedestal == 1 then
			unmovableIndex = 4
			humanoidRoot.Anchored = true
			savedPlayerPos = char:GetPrimaryPartCFrame()
