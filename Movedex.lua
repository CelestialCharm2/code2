	...
	['dragoncheer'] = {
		...
		condition = {
			onStart = function(target, source, effect)
				if target:hasVolatile('focusenergy') then return false end
				if effect then
					self:add('-start', target, 'move: Dragon Cheer')
				end
				self.effectState.hasDragonType = target:hasType("Dragon")
			end,
			onModifyCritRatio = function(self, critRatio, source)
				return critRatio + (self.effectState.hasDragonType and 2 or 1)
			end,
		}
	},
	-- dont change
