		...
		while #minors > 0 do
			...
			elseif arg1 == '-start' then
				...
				elseif effect.id == 'powder' then
					...
					addAction((poke and poke:getName() or "Pokemon") .. "'s " .. (effect and effect.name or "Effect") .. " started!")
				end
				poke.side:updateStatbar()
			elseif arg1 == '-end' then
				...
				if args[3] == 'Dynamax' then
					...
					self:stopZPowerGlow()			
					addAction(poke:getName() .. " returned to normal!")
				end
				if kwargs.silent then
					-- do nothing
				elseif effect.id == 'powertrick' then
	...
	function Battle:runMajor(args, kwargs)
		...
		elseif arg1 == '-start' then
			...
			if args[3] == 'Dynamax' or args[3] == 'maxRaid' then
				local pokemon = self:getPokemon(args[2])
				local move = args[3]
				--	pokemon.sprite:animDmax(pokemon, self)
				...
				if poke.name == 'Calyrex' then self:startZPowerGlow(poke.sprite, 0.6576) else self:startZPowerGlow(poke.sprite, 0) end
				self:message(poke:getName() .. "'s Dynamax!")
			end
