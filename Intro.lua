return function(_p)
	...
	function intro:newGame(fader)
		...
		_p.DataManager:preload(13206650540, 13206658822, 13206665797)
		Utilities:layerGuis()
		...
		-- [[
		Utilities.sound(18721681714, nil, nil, 35)--288901896, nil, .5, 35)
	...
	function intro:newPlayerInfo()
		...
		write([[Some notes from the developers of Project Bronze Eternal:
 ...
 - Project Bronze Eternal is still in development. There will be bugs here and there but we're always working to fix them. Help us out by sending a message on our group wall or in our discord server!]]) {
			Frame = textbox,
			Size = textbox.AbsoluteSize.Y*.0375,
			Wraps = true,
		}
		write([[- If you have any questions about Project Bronze Eternal, feel free to join us at https://discord.gg/pbe for more info!]]) {
			Frame = textbox2,
			Size = textbox.AbsoluteSize.Y*.0375,
			Wraps = true,
		}
	...
	function intro:perform(loadGui, loadFn)
		...
		pcall(function() --if game.CreatorId == 6803112 or game:GetService('RunService'):IsServer() then
			...
			skipButton =  _p.RoundedFrame:new {
				BackgroundColor3 = Color3.new(0, 0.498039, 0.745098),
		...
		if not skipped then
			...
			fireAnimBottom:Play()
			hoopa = animation:new {sheets={{id=5795538473,rows=3},{id=5795538045,rows=3},{id=5795537531,rows=3},{id=5795537046,rows=3},},nFrames=69,fWidth=86,fHeight=94,framesPerRow=6}
		...
		local pokemon = create("ImageLabel")({
			BackgroundTransparency = 1,
			Image = "rbxassetid://13206565198", -- 771144807
			ZIndex = 5,
		...
		for _, lb in pairs(bbLetterBounds) do
			do
				...
				local letter = create("ImageLabel")({
					BackgroundTransparency = 1,
					Image = "rbxassetid://13206561162", -- 771148059
		...
		local cm = Utilities.loopSound('rbxassetid://18721673643', .6)--.4)
		...
		if choice == 'newgame' then
			...
			local enableautosave
			wait(0.8)
			enableautosave = _p.NPCChat:say('[y/n]Would you like to enable Autosave?')
