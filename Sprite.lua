return function(_p)
	...
	function Sprite:animateMarshadowAttack()
		local pokemon = self.pokemon
		spawn(function()
			self.spriteData = _p.DataManager:getSprite((pokemon.shiny and '_SHINY' or '')..(self.isBackSprite and '_BACK' or '_FRONT'), 'Marshadow-Attack')
	...
	-- transform
	function Sprite:renderNewSpriteData(playOnce)
        if self.animation then
            ...
            local a = _p.AnimatedSprite:New(sd, true)
	...
	function Sprite:animDmax(poke, battle)
		...
		local Dfielf = _p.storage.Models.Misc.Dmax:Clone()
		local main = Dfielf.Main
		local DmaxBall = Dfielf.DynamaxBall
		local zp = main.Position
		Dfielf.Parent = workspace

		sprite.cf = CFrame.new(zp) + Vector3.new(0, main.Size.Y / 2, 0)
		local trainer = self:getTrainer()
		local cp = Dfielf.PlayerPos.Position + Vector3.new(0, Dfielf.PlayerPos.Size.Y , 0)
		...
		cam.CFrame = Dfielf.Playercam.CFrame
		local animtrack = trainer.MaxThrow

--[[		Tween(2, nil, function(a)
			MoveModel(DmaxBall.Main, DmaxBall.Main+ Vector3.new(4* a, 0, 0))
		end)	]]
		wait(1)
		Utilities.Tween(1, nil, function(a)
			DmaxBall.CFrame = Dfielf.ballpos1.CFrame:Lerp(Dfielf.ballpos2.CFrame, a)
		end)
		--	animtrack:Play(0)
		Utilities.Tween(1, nil, function(a)
			DmaxBall.CFrame = Dfielf.ballpos2.CFrame:Lerp(Dfielf.ballpos3.CFrame, a)
		end)
		wait(1)
		animtrack:Stop(0)
		pcall(function()
		...
		cam.FieldOfView = battle.battleCamera.FieldOfView --Change to previous FOV for zMoves
		for _, g in pairs(disabledGuis) do
			pcall(function()
				g.Visible = true
			end)
