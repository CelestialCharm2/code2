...
local function GetData(id)
	...
	return _f.PlayerDataService[plr], plr
end
function TradeManager:logTrade(playerId, partnerId, playerOffer, partnerOffer)

	local PDS, plr = GetData(playerId)
	local partnerPDS, partner = GetData(partnerId)
	local logData = {
		{
			plr = plr,
			offer = {},
	...
	local function copyToOffer(num, poke)
		local data = {}
		for i=1, #copyValues do
			local val = copyValues[i]
			data[val] = poke[val]
		end
		table.insert(logData[num].offer, data)
	end

	for i, pokemon in pairs(playerOffer) do
		local poke = _f.ServerPokemon:deserialize(pokemon, PDS)
		copyToOffer(1, poke)
	end
	for i, pokemon in pairs(partnerOffer) do
		local poke = _f.ServerPokemon:deserialize(pokemon, partnerPDS)
		copyToOffer(2, poke)
	end
...
function TradeManager:trade(player, playerOffer, partnerOffer)
	...
	if self.p1trade and self.p2trade then
		...
		local p1_pokemon_data = {}
		local p2_pokemon_data = {}
		...
		for i,pokemon in pairs(p1serOffer) do
			...
			deserializepokemon.item = item
			table_insert(p1_pokemon_data, deserializepokemon)
		end
		...
		for i,pokemon in pairs(p2serOffer) do
			...
			deserializepokemon.item = item
			table_insert(p2_pokemon_data, deserializepokemon)
		end
		...
		_f.Logger:logTrade(game.Players[p1name], game.Players[p2name], p1_pokemon_data, p2_pokemon_data)
	end
end
