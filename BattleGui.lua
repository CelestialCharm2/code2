return function(_p)
	...
	function BattleGui:updateButtonForZMove(button, move, zmove, zmoves)
		...
		local ZMoveColors = {
			...
			["Gigavolt Havoc"] = Color3.fromRGB(253, 237, 7)
		};
	...
	function BattleGui:animAbility(poke, abilityName)
		...
		local gui = create 'ImageLabel' {
			BackgroundTransparency = 1.0,
			Image = 'rbxassetid://'..({13607072331, 13607075875})[n],
	...
	function BattleGui:animMove(battle, pokemon, move, targets)
		if pokemon.species == 'Marshadow' then
			pokemon.sprite:animateMarshadowAttack() 
		end
	...
	function BattleGui:mainChoices(...)
		...
		self.mainButtonClicked = function(name)
			...
			if name == 'Fight' then
				...
				if name == 'Run' then --All these fire when you click run
					...
					if battleKind == 'pvp' or battleKind == '2v2' then
						...
						spawn(function() self:exitButtonsMain() end)--removes fight ect.
						...
						if self:message('[y/n]Are you sure you want to forfeit this match?') then
							...
							return self:mainChoices(unpack(args)) --returns fight ect
						end
				...
				elseif name == 'Bag' then
					if _p.Battle.currentBattle.kind == 'pvp' or _p.Battle.currentBattle.kind == '2v2' then
						state = 'canchoosemain'
						return							
					elseif _p.Battle.currentBattle.cantUseBag == true then
						self:message("You cannot use your bag in this battle!")
						state = 'canchoosemain'
						return 
					end
		...
		if isFirstValid then
			...
			elseif _p.Battle.currentBattle.isSafari then
				if fight.Name ~= 'Ball' then
					fight.Name = 'Ball'
					fight.txt:ClearAllChildren()
					write 'Ball' {
						Frame = fight.txt,
						Scaled = true,
						Color = Color3.new(.4, .15, .15),
					}
				end
				if bag.Name ~= 'Berry' then
					bag.Name = 'Berry'
					bag.txt:ClearAllChildren()
					write 'Berry' {
						Frame = bag.txt,
						Scaled = true,
						Color = Color3.new(.4, .25, .15),
					}
				end
				if pokemon.Name ~= 'Go Near' then
					pokemon.Name = 'Go Near'
					pokemon.txt:ClearAllChildren()
					write 'Go Near' {
						Frame = pokemon.txt,
						Scaled = true,
						Color = Color3.new(.15, .4, .25),
					}
				end
			end
	...
	function BattleGui:chooseMoveTarget(moveNum, move, rqPokemon, userPosition, nActive)
		...
		function self.onTargetClicked(n)
			if state ~= "canchoosetarget" then
				return
			end
			if getTokenForPosition(n) > 0 then
				...
				self.inputEvent:fire('move '..moveNum.. ' '..t..((gui.mega.selected and ' mega') or (gui.zmove.selected and ' zmov') or ''))
			...
			for i = 1, 6 do
				targets[i] = create 'ImageLabel' { -- 522 x 130
					...
					create 'ImageLabel' { -- 537x140
						Name = 'HighlightIcon',
						Image = 'rbxassetid://14507685043', --yw btw
	...
	function BattleGui:fightChoices(moveset, rqPokemon, slot, nActive, alreadyChoseMega)
		...
		self.onMoveClicked = function(m)
			...
			if not gui.zmove.selected then
				...
				if not vaildzmoves[m] then return end
			end
			...
			function self.moveUpdate()
				...
				for i = 1, 4 do
					if zmove.selected then
						...
						vaildzmoves[i] = self:updateButtonForZMove(moves[i], moveset[i], z, zMoves)
					else
						-- Update move button
						vaildzmoves = {}
	...
	function BattleGui:animateEvolution(name1, name2, sd1, sd2, alreadyFaded, cannotBeCanceled)
		...
		delay(1, function()
			...
			sound = Utilities.loopSound(13011897719)
		end)
		...
		spawn(function()
			...
			for i = 1, n do
				...
				local image = Utilities.Create 'ImageLabel' {
					BackgroundTransparency = 1.0,
					Image = 'rbxassetid://13607143521',--576575353
		...
		Utilities.sound(13011903517, nil, .5, 10)
