...
BattlePokemon = class({
	...
	if set.gigantamax then self.gigantamax = true end
...
function BattlePokemon:maxMoveDisabled(baseMove)
	...
	if not (self:getMoveData(baseMove.id).pp) then return true end
...
function BattlePokemon:getUndynamaxedHP(amount) 
	...
	return (hp and self.baseMaxhp and self.maxhp and self.maxhp ~= 0) and math.ceil(hp * self.baseMaxhp / self.maxhp) or hp
end
...
function BattlePokemon:getRequestData() -- for pokemon in request.active (for request.side, see BattleSide:getData())
	...
	if not lockedMove then --Cannot do these actions if the move is locked
		...
		if self.canDynamax and self:getDynamaxRequest() then
			...
			data.maxMoves = self:getDynamaxRequest().maxMoves--self.maxMoves
			--[[
				Need to rewrite BattleGUI for maxMoves
			]]
		end
