...
--]]
local ITEM_SHEET_ID = 17136215200
...
return function(_p)--local _p = require(script.Parent.Parent)--game:GetService('ReplicatedStorage').Plugins)
	...
	function bag:getItemIcon(item)
		...
		if type(item)=='number' then
			...
			if item.name:sub(1, 2) == 'TM' or item.name:sub(1, 2) == 'HM' or item.name:sub(1, 2) == 'TR' then
		...
		local itemToAsset = {
			[734] = 15880798448, --sceptilite C
			[669] = 15880802593, --Z-Power ring
			[758] = 15880808503, --Zygarde Cube
			[759] = 15880813519, --dmax Band
			[760] = 15880817418, --Beast Ball
			[787] = 15880822678, --// Christmas Lights
			[788] = 15880825938, --// White Leaf
			[829] = 91781891808100, --// Strange amulet
		}
		...
		return create 'ImageLabel' {
			...
			Image = 'rbxassetid://'..(not hasItemToAsset() and ITEM_SHEET_ID or tostring(itemToAsset[icon])),
			ImageRectSize = not hasItemToAsset() and Vector2.new(32, 32) or Vector2.new(0, 0),
			ImageRectOffset = not hasItemToAsset() and Vector2.new(32*((icon-1)%26), 32*math.floor((icon-1)/26)) or Vector2.new(0, 0),
	...
	function bag:updatePartyList(pouch, specificPokemonIndex, context)--::updatePartyList
		if pouch == 4 then
			return
		end
		local tm, zcrystal
		if type(pouch) == "table" then
			if pouch.compat then
				zcrystal, pouch = pouch, nil
			else
				tm, pouch = pouch, nil
			end
		end
		fastSpawn(function()
			...
			-- update gui
			local function update(i, context)
				if context == 'fuse' then
					party = _p.Network:get('PDS', 'getParty', 'bag')
					rewriteNames = true
					for i, p in pairs(party) do p.index = i end
					self.partyData = party
				...
				if pokemon then
					...
					if pouch and not pokemon.egg then
					...
					elseif zcrystal then
						local compat = pokemon.itemId == zcrystal.id and 2 or zcrystal.compat[i]
						local text, color = unpack(({
							{
								"Incompatible",
			...
			if specificPokemonIndex then
				update(specificPokemonIndex, context)
			else
	...
	function bag:updatePartyForZCrystalCompatibility(crystal)
		if self.sellEvent then
			return
		end
		self.updatePartyOnDeselect = true
		self:updatePartyList(crystal)
	end
	...
	function bag:selectItem(item, gui, pouch)--::selectItem
		...
		elseif pouch == 6 then
			self:updatePartyForZCrystalCompatibility(item)
		end
	end
	...
	function bag:useSelectedItem()--::useSelectedItem
		...
		if not usableItem or not usableItem.noTarget then
			pokemon, Index, bg, txtRF, cancel = self:choosePokemonAndMove("Use on", usableItem and ((usableItem.chooseMove and 'Move' or usableItem.chooseIVs and 'Ivs') or false))
		end
		local use = false
		if pokemon or (usableItem and usableItem.noTarget) then
			...
			elseif (usableItem and usableItem.canUse and (usableItem.canUse == true or usableItem.canUse(pokemon, Index))) -- determined by client
				...
				use = true
				local smsg, updatedTarget = _p.Network:get('PDS', 'useItem', item.id, (pokemon and pokemon.index), Index)
				...
				elseif usableItem and usableItem.onProcess then
					usableItem.onProcess(updatedTarget or pokemon, smsg, Index)
				...
				if use and usableItem and usableItem.onUse then
					local cmsg = usableItem.onUse(pokemon, Index)
					if cmsg then
						chat:say(cmsg)
		...
		if use then
			for i=1, 6 do 
				self:updatePartyList(self.currentPouch, i, 'fuse')
			end
			self:updateCurrentPouch()
			self:deselect()
		end
	...
	function bag:giveSelectedItem()--::giveSelectedItem
		...
		local bg = create 'ImageButton' {
			...
			BorderSizePixel = 0,
			BackgroundTransparency = 0.5,
			BackgroundColor3 = Color3.new(0, 0, 0),--Bag select colour
			Size = UDim2.new(1.0, 0, 1.0, 0),
			ZIndex = 6, Parent = Utilities.gui,
		}
		local txtRF = _p.RoundedFrame:new {
	...
	function bag:updateListForPouch(n)--::updateListForPouch
		...
		fastSpawn(function()
			...
			for i, item in pairs(pouch) do
				if n == 6 then item.qty = nil end
				local gui; gui = create 'ImageButton' { -- 150x100
					BackgroundTransparency = 1.0,
					Image = 'rbxassetid://15880843464',--290486212
	...
	function bag:updateListForSafari()
		local bags
		fastSpawn(function()
			bags = _p.Network:get('PDS', 'getSafariBag')
		end)
	end
	...
	function bag:open(battleEvent, sellEvent, chooseEvent)
		...
		if not backdrop then
			...
			backdrop = create 'ImageLabel' {
				Name = 'BagBackdrop',
				BackgroundTransparency = 1.0,
				Image = 'rbxassetid://95345284106560',
				SizeConstraint = Enum.SizeConstraint.RelativeYY,
				Size = UDim2.new(1.0, 0, 1.0, 0),
			...
			for i, c in pairs({'Items','Medicine','Poke Balls','TMs/HMs','Berries','Key Items', 'Z-Crystals'}) do
				...
				if i == 7 and not zpouchalreadyenable then --// Z-Move
			...
			self.zCrystalPouchButton = buttons[7]
			local cancel = _p.RoundedFrame:new {
				Button = true,
			...
			local options = _p.RoundedFrame:new {
				BackgroundColor3 = Color3.new(.2,.2,.2), -- .9,.625,0
		...
		if not self.inBattle then
			...
			if _p.DataManager.currentChunk.id == "chunk97" then
				self:updateListForSafari()
			else
