return function(_p)
	...
	local mapData = { -- total 586 586
		...
		{'Cosmeos Valley',  1, .71, .36, 'chunk54', 'vCosmeos', 'Gate21'},
		{'Grove Of Dreams', 1., .24, .58, 'chunk14', 'vGrove', 'Gate12'}
	}

	local nmapData = {
		{'MapTest', 1, .85, .60, 'chunk1'}
	}
	...
	function map:fly()
		...
		if _p.PlayerData.completedEvents.eventthatdoesntexistyet then--_p.PlayerData.badges[4] and _p.PlayerData.completedEvents.purpleGlow then
			local New = _p.RoundedFrame:new {
				Button = true,
				BackgroundColor3 = BrickColor.new('Royal purple').Color,
				Size = UDim2.new(.31, 0, .08, 0),
				Position = UDim2.new(.52, 0, -.03, 0),
				ZIndex = 9, Parent = mapGui,
				MouseButton1Click = function()
					if self.weatherVari then
						self.weatherVari:Remove()
					end
					for i,v in pairs(mapGui:GetDescendants()) do
						local ya,na = pcall(function()
							if v.Size == UDim2.new(.055, 0, .055, 0) then v:Remove() end
						end)
					end
					for i,v in pairs(container:GetDescendants()) do
						local ya,na = pcall(function()
							if v.Size == UDim2.new(1.0, 0, 1.0, 0) then v.Image = 'rbxassetid://'..nmapId end
						end)
					end
					for _, d in pairs(nmapData) do
						local rEvent = d[6]
						local enabled = not rEvent or _p.PlayerData.completedEvents[rEvent]
						local i; i = create 'ImageButton' {
							BackgroundTransparency = 1.0,
			...
			Utilities.Write 'MapTest' {
				Frame = create 'Frame' {
					BackgroundTransparency = 1.0,
					Size = UDim2.new(1.0, 0, 0.7, 0),
					Position = UDim2.new(0.0, 0, 0.15, 0),
					ZIndex = 10, Parent = New.gui
				}, Scaled = true,
			}
			local roria = _p.RoundedFrame:new {
				Button = true,
				BackgroundColor3 = BrickColor.new('Baby blue').Color,
				Size = UDim2.new(.31, 0, .08, 0),
				Position = UDim2.new(-.12, 0, -.03, 0),
				ZIndex = 9, Parent = mapGui,
				MouseButton1Click = function()
					if self.weatherVari then
						self.weatherVari:Remove()
					...
					for i,v in pairs(container:GetDescendants()) do
						local ya,na = pcall(function()
							if v.Size == UDim2.new(1.0, 0, 1.0, 0) then v.Image = 'rbxassetid://'..mapId end
						end)
					end
					for _, d in pairs(mapData) do
						local rEvent = d[6]
			...
			Utilities.Write 'Roria' {
				Frame = create 'Frame' {
					BackgroundTransparency = 1.0,
					Size = UDim2.new(1.0, 0, 0.7, 0),
					Position = UDim2.new(0.0, 0, 0.15, 0),
					ZIndex = 10, Parent = roria.gui
				}, Scaled = true,
			}
		end
