return function(_p)
	...
	function options:getUnstuck(manually)
		...
		elseif _p.context == 'trade' then
			cf = CFrame.new(57, 2, 52) + Vector3.new(math.random()*40-20, 0, math.random()*40-20)
		else
	...
	--// Get Void Unstuck
	function options:voidUnstuck()
		if tick()-self.lastUnstuckTick < unstuckCooldown then return end
		local chunk = _p.DataManager.currentChunk
		_p.Hoverboard:unequip(true)
		if _p.Surf.surfing then
			_p.Surf:forceUnsurf()
		end
		local chunkCFrame = CFrame.new(-79.1, 73.335, -654.5)
		Utilities.FadeOut(.5, Color3.new(0, 0, 0))
		Utilities.TeleportToSpawnBox()
		chunk:Remove()
		task.wait(.5, _p.DataManager:loadChunk('chunk2'))
		task.wait(.5, Utilities.Teleport(chunkCFrame))
		task.wait(.5, Utilities.FadeIn(.5))
		self.lastUnstuckTick = tick()
	end
	...
	function options:open()
		...
		if not gui then
			...
			TextSpeed.ValueChanged:connect(function()
				_p.NPCChat.speed = TextSpeed.Value and 100000*1000 or 100000
			end)
