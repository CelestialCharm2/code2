...
return function(_p)--local _p = require(script.Parent)
	...
	--- FirebaseDev's Custom Functions ---
	-----------------------------------

	--// Tween System
	local function TweenCamera(cam, duration, Cframe)
	...
	local function onObtainBadgeSound()
		Utilities.sound(10876393946, nil, nil, 10)
	end
	
	local function badgeSpin(badgeModel, badgeName, isBottom)
		...
		chat.bottom = isBottom
		chat:say('Obtained the '..badgeName..' Badge!')
		chat.bottom = nil     
	...
	local function TweenFunction(object ,timer, ease ,items)
		local TS = game:GetService('TweenService')
		local tween = TS:Create(object, TweenInfo.new(timer, ease), items)
		tween:Play()
	...
	return {
		...
		onLoad_colosseum = function(chunk)
			...
			interact[bottlecap.model] = function()
				chat:say(bottlecap, 'Yo what\'s up, Hyper Trainer here.',
					'With my Hyper Training, you can boost your Pokemon\'s stats!'
				)
				local r, r2 = _p.Network:get('PDS', 'hasbottlecaps')
				if chat:say(bottlecap, '[y/n]Just pay a price of Bottle Caps! Wanna try?') then
			...
			interact[moveTutor.model] = function() 
				...
				local moveTutor = function()
					...
					local payment = chat:choose('30 BP', '[$]30,000', 'Cancel')
					frame:Remove()
					-- if cannot afford any

					if payment == 1 and r.bp < 50 then
						moveTutor:Say("Oh you dont have enough BP make sure to come back when u have moreee!")                           
						return
					...
					if payment == 1 then
						moveTutor:Say("I'll take that 30 BP now.")
					elseif payment == 2 then
						moveTutor:Say("I'll take that [$]30,000 now.")
					end
		...
		onLoad_chunk1 = function(chunk)
			local caveblock = chunk.map.OtherStuff.CaveBlock
			local caveguard1 = chunk.map.OtherStuff.CaveGuard1
			local caveguard2 = chunk.map.OtherStuff.CaveGuard2
			local caveguard3 = chunk.map.OtherStuff.CaveGuard3
			local caveguard4 = chunk.map.OtherStuff.CaveGuard4

			if completedEvents.eventthatdoesntexistyet then 
				caveblock:Remove()
				caveguard1:Remove()
				caveguard2:Remove()
				caveguard3:Remove()
		...
		onExit_chunk666 = function(chunk)
			local lighting  = game:GetService("Lighting")
		...
		onDoorFocused_Gate1 = function()
			...
			local music = Utilities.loopSound(18721719712)
		...
		onLoad_chunk2 = function(chunk)
			local function setupForestBattle()
				_p.DataManager:preload(6860109291, 13488406831)
		...
		onBeforeEnter_House6 = function(room)
			local inGroup = _p.player:IsInGroup(35034961)
		...
		onLoad_chunk3 = function(chunk)
			if completedEvents.eventthatdoesnotexist then
				chunk.map.rocks:Remove()
			end

			local defaultio = chunk.npcs.Defaultio
			local MewtwoBlock = chunk.map.blockyblock
		...
		onLoad_chunk7 = function(chunk)
			...
			if completedEvents.GroudonScene then
				...
				_p.DataManager:preload(12975910326, 6860109291, 10841856155, 10841858409,10841860027, 68068592, 317480860) -- admin music, eclipse logo, earthquake, groudon scene music[2], particle, pulse
		...
		onBeforeEnter_DayCare = function(room)
			...
			interact[dcl.model] = function()
				...
				dcl:Say('Welcome to the Pokemon Project Bronze Eternal Day Care.', 'We\'re happy to help raise your pokemon!',
...
--[[					if #_p.PlayerData.party == 6 and not _p.PlayerData.pc:hasSpace() then  -- todo?
		...
		onLoad_chunk11 = function(chunk)
			...
			interact[krg.model] = function()
				...
				krg:Say('The King\'s Rock is a symbol of strength. Once owned by the king Devman848',
			...
			if completedEvents.LighthouseScene then
				...
				connectLighthouseScene = function()
					...
					touchEvent('LighthouseScene', lhtrigger, false, function()
						...
						local win = _p.Battle:doTrainerBattle {
							IconId = 6860109291,
							musicId = {13059073641,13061651452},
							musicVolume = 2,
							PreventMoveAfter = true,
							LeaveCameraScriptable = true,
						...
						_p.DataManager:preload(337448917, 243728104, 13068313488, 13068313488, 337973384, 317480860, 338262406)
				...
				chatCn = _p.player.Chatted:connect(function(msg)
					...
					if MasterControl.WalkEnabled and not _p.DataManager.isDay and msg:lower():match('^i%s+wish%s+%a') then
						...
						--]]
							...
							_p.Battle:doWildBattle(_p.DataManager.currentChunk.regionData.Wish,{battleSceneType = 'Jirachi', musicId = 13488182400})
		...
		onLoad_chunk16 = function(chunk)
			local Blocking = chunk.map.Blocking
			if completedEvents.RayquazaInformation then
				Blocking:Remove()
			end
		end,
...
--				blimpEngine.Anchored = true
		...
		onBeforeEnter_LottoShop = function(room)
			...
			interact[hobo.model] = function()
				...
				elseif triesToday > 0 then
					if not hobo:Say("So far, you've drawn " .. triesToday .. " ticket" .. (triesToday == 1 and "" or "s") .. " today.", "You can draw " .. 4 - triesToday .. " more ticket" .. (triesToday == 3 and "" or "s") .. " for 30 R$" .. (triesToday == 3 and "." or " each."), "[y/n]Would you like to purchase a ticket for 30 R$?") then
						hobo:Say("Please do visit again!")
						return
					end
				end
		...
		onLoad_chunk20 = function(chunk) -- ANTHIAN: SHOPPIN
			...
			-- Shadow Mewtwo Dealer
			--	local dealer = chunk.npcs.shadowmewtwodealer
			--	interact[dealer.model] = function()
			--		local buy = dealer:Say(
			--			'Ah, greetings, aspiring trainer.',
			--			'I see you possess an eye for rare Pokémon, do you not?',
			--			'No? Perhaps I misjudged. However, I possess a creature of remarkable distinction, though not of a particularly rare breed.',
			--			'What sets it apart is its extraordinary looks.  Quite remarkable, wouldn\'t you agree?',
			--			'et, it seems you\'re not fully captivated by this Pokemon\'s potential. No matter. I\'m willing to part with it for a modest fee of 1000 ROBUX. Consider it a token of appreciation for your future endeavors. Of course, you\'ll need to ensure to save our transaction once concluded.',
			--			'[y/n]So, what do you say? Are you intrigued enough to seize this opportunity?')
			--		if not buy then
			--			dealer:Say('Yeah, I didn\'t think so...', 'Sorry to waste your time.')
			--			return
			--		end
			...
			--		local loadTag = {}
			--		_p.DataManager:setLoading(loadTag, true)
			--		local r = _p.Network:get('PDS', 'buyShadowMewtwo')
			--		_p.DataManager:setLoading(loadTag, false)
			--		chat:manualAdvance()
			--		if not r then
			--			dealer:Say('Your payment failed.', 'Let me know if you get serious about this deal.')
			...
			--			chat.bottom = true
			--			Utilities.sound(304774035, nil, nil, 8)
			--			chat:say('Obtained Shadow Mewtwo!')
			--			local nickname
			--			if dealer:Say('[y/n]Would you like to give a nickname to Mewtwo?') then
			--				nickname = _p.Pokemon:giveNickname(r.i, r.s)
			--			end
			--			local msg = _p.Network:get('PDS', 'makeDecision', r.d, nickname)
			--			if msg then chat:say(msg) end
		...
		onLoad_chunk22 = function(chunk, data)
			...
			if _p.PlayerData.badges[4] and not completedEvents.DefeatTEinAC then
				...
				spawn(function()
					...
					_p.DataManager:preload(280857070, 10841914611, 507289472, 334858056, 509072758, 509073816, 13072999208)
					--]]
		...
		onLoad_BattleFrontier = function(chunk)
			local salesperson = chunk.npcs.BerryShop
			interact[salesperson.model] = function()
				spawn(function() _p.Menu:disable() end)
				chat:say(salesperson, 'Welcome to the Berry Shop!', 'May I help you?')
				while true do
			...
			local salesperson = chunk.npcs.GemShop
			interact[salesperson.model] = function()
				spawn(function() _p.Menu:disable() end)
				chat:say(salesperson, 'Welcome to the Gem Stone Shop!', 'May I help you?')
				while true do
					if not _p.Menu.shop:open('gemshp') then break end
					chat:say(salesperson, 'Is there anything else I may do for you?')
				end
				chat:say(salesperson, 'Please come again!')
				_p.Menu:enable()
			end
			local salesperson = chunk.npcs.StoneShop
			interact[salesperson.model] = function()
				spawn(function() _p.Menu:disable() end)
				chat:say(salesperson, 'Welcome to the Stone Shop!', 'May I help you?')
				while true do
			...
			local salesperson = chunk.npcs.MegaShop
			interact[salesperson.model] = function()
				spawn(function() _p.Menu:disable() end)
				chat:say(salesperson, 'Welcome to the Mega Stone Shop!', 'May I help you?')
				while true do
					if not _p.Menu.shop:open('megashp') then break end
					chat:say(salesperson, 'Is there anything else I may do for you?')
				end
				chat:say(salesperson, 'Please come again!')
				_p.Menu:enable()
			end
			local salesperson = chunk.npcs.Training
			interact[salesperson.model] = function()
				spawn(function() _p.Menu:disable() end)
				chat:say(salesperson, 'Welcome to the Training Shop!', 'May I help you?')
				while true do
					if not _p.Menu.battleShop:open() then break end
					chat:say(salesperson, 'Is there anything else I may do for you?')
				end
				chat:say(salesperson, 'Please come again!')
				_p.Menu:enable()
			end
			local salesperson = chunk.npcs.Competitive
			interact[salesperson.model] = function()
				spawn(function() _p.Menu:disable() end)
				chat:say(salesperson, 'Welcome to the Competitive Shop!', 'May I help you?')
				while true do
					if not _p.Menu.battleShop2:open() then break end
					chat:say(salesperson, 'Is there anything else I may do for you?')
				end
				chat:say(salesperson, 'Please come again!')
				_p.Menu:enable()
			end
			local salesperson = chunk.npcs.Incense
			interact[salesperson.model] = function()
				spawn(function() _p.Menu:disable() end)
				chat:say(salesperson, 'Welcome to the Incense Shop!', 'May I help you?')
				while true do
					if not _p.Menu.battleShop3:open() then break end
					chat:say(salesperson, 'Is there anything else I may do for you?')
				end
				chat:say(salesperson, 'Please come again!')
				_p.Menu:enable()
			end
			local salesperson = chunk.npcs.Mint
			interact[salesperson.model] = function()
				spawn(function() _p.Menu:disable() end)
				chat:say(salesperson, 'Welcome to the Mint Shop!', 'May I help you?')
				while true do
					if not _p.Menu.battleShop4:open() then break end
					chat:say(salesperson, 'Is there anything else I may do for you?')
				end
				chat:say(salesperson, 'Please come again!')
				_p.Menu:enable()
			end
			local salesperson = chunk.npcs.Tms
			interact[salesperson.model] = function()
				spawn(function() _p.Menu:disable() end)
				chat:say(salesperson, 'Welcome to the TM Shop!', 'May I help you?')
				while true do
					if not _p.Menu.battleShop5:open() then break end
					chat:say(salesperson, 'Is there anything else I may do for you?')
				end
				chat:say(salesperson, 'Please come again!')
				_p.Menu:enable()
			end
			local healer = chunk.npcs.Nurse
			interact[healer.model] = function()
				spawn(function() _p.Menu:disable() end)
				MasterControl.WalkEnabled = false
				local healed = chat:say(healer, 'Hello Trainer!', '[y/n]Would you like to rest your pokemon?')
				if not healed then
					chat:say(healer, 'Oh alright, I understand. Come back later if you need anything.') spawn(function() _p.Menu:enable() end) 
					MasterControl.WalkEnabled = true
				else
					_p.Network:get('PDS', 'getPartyPokeBalls')
					chat:say(healer, 'OK. We\'ve restored your pokemon to full health.',
		...
		onLoad_chunk51 = function(chunk)
			...
			touchEvent('Regigigas', chunk.map.Trigager, true, function()
				_p.Hoverboard:unequip(true)
				spawn(function() _p.Menu:disable() end)
				MasterControl.WalkEnabled = false
				MasterControl:Stop()
		...
		onLoad_chunk58 = function(chunk)
			_p.DataManager:preload(_p.animationId.Rayquaza)
			_p.DataManager:preload(_p.animationId.Groudon)
			_p.DataManager:preload(_p.animationId.Kyogre)
			...
			interact[captain.model] = function()
				...
				local darkrai = _p.Network:get('PDS', 'nightmareticket')
				if not has then
				...
				if not completedEvents.TalkToCap or not has then
					...
					if darkrai then
						table.insert(options, 'New Moon Island')
					end
					if #options > 0 then
						spawn(function() _p.Menu:disable() end)
						...
						if choice == 'New Moon Island' then
							chat:say(captain, 'All aboard!')
							Utilities.FadeOut(.5)
							local startTick = tick()
							--								flying = false
							...
							chunk:Remove()
							-- change chunks
							_p.DataManager:loadChunk('chunk3005') -- New Moon Island

							--workspace.CurrentCamera.CameraType = Enum.CameraType.Custom
							Utilities.Teleport(CFrame.new(-567.091, 13.492, -48.517))
							local elapsed = tick()-startTick
							if elapsed < .5 then
								wait(.5-elapsed)
							end
			...
			local cam = workspace.CurrentCamera
			local weathertrio = chunk.map.WeatherTrio
			local telescopedudee = chunk.npcs.telescopedudee
			if completedEvents.WeatherTrio then weathertrio:Remove() telescopedudee:Remove() return end
			if not _p.PlayerData.badges[8] or not completedEvents.HasRedAndBlueOrb then weathertrio:Remove() telescopedudee:Remove() return end
			if not completedEvents.WeatherTrio and completedEvents.HasRedAndBlueOrb then 			
				local lighting = game:GetService("Lighting")
				_p.DataManager:lockClockTime(0)
				lighting.TimeOfDay = "00:00:00" 
			end
			local Kyogre = chunk.map.WeatherTrio.Kyogre
			local kyogre = Kyogre.AnimationController:LoadAnimation(create 'Animation' { AnimationId = 'rbxassetid://'.._p.animationId.Kyogre })
			local Groudon = chunk.map.WeatherTrio.Groudon
			local groudon = Groudon.AnimationController:LoadAnimation(create 'Animation' { AnimationId = 'rbxassetid://'.._p.animationId.Groudon })	
			local Rayquaza = chunk.map.WeatherTrio.Rayquaza
			local rayquaza = Rayquaza.AnimationController:LoadAnimation(create 'Animation' { AnimationId = 'rbxassetid://'.._p.animationId.Rayquaza })
			spawn(function() 
				rayquaza:play()  
				kyogre:play() 
				groudon:play()
			end)
			interact[telescopedudee.model] = function()
				spawn(function() _p.Menu:disable() end)
				--	local walkto = chunk.map.walkto.Position
				MasterControl.WalkEnabled = false
				if telescopedudee:Say('[y/n]Are you ready to Shiny Hunt?') then
					spawn(function() _p.PlayerData:completeEvent('WeatherTrio') end)
					_p.Battle:doWildBattle(_p.DataManager.currentChunk.regionData.Rayquaza, {battleSceneType = 'Rayquaza'})
					telescopedudee:Remove() weathertrio:Remove()
				else telescopedudee:Say('You finally made it, While you were on your way here I already summoned both Groudon and Kyogre',
					'Come with me now so we can help them stop Rayquaza'
					)
					wait(.5)
					local pos = {
						chunk.map.walkto1.Position,
						chunk.map.walkto2.Position,
						chunk.map.walkto3.Position,
						chunk.map.telescopewalkto.Position
					}
					spawn(function()
						wait(.6)
						for _, node in pairs(pos) do
							telescopedudee:WalkTo(node)
						end 
					end)
					local poss = {
						chunk.map.walkto1.Position,
						chunk.map.walkto2.Position,
						chunk.map.walkto3.Position,
						chunk.map.playerwalkto.Position
					}
					spawn(function()
						wait(.6)
						for _, node in pairs(poss) do
							MasterControl:WalkTo(node)
						end 
					end)
					local p15 = chunk.map.part15.CFrame
					Utilities.FadeOut(.4)
					cam.CameraType = Enum.CameraType.Scriptable
					cam.CFrame = p15
					wait(.1)
					...
					wait(2.5)
					Utilities.FadeOut(.4)
					local p1 = chunk.map.part1.Position
					local p2 = chunk.map.part2.Position
					local p3 = chunk.map.part3.Position
					local p4 = chunk.map.part4.Position
					local p5 = chunk.map.part5.Position
					local p6 = chunk.map.part6.Position
					local p7 = chunk.map.part7.Position
					local p8 = chunk.map.part8.Position
					local p9 = chunk.map.part9.Position
					local p10 = chunk.map.part10.Position
					local p11 = chunk.map.part11.Position
					local p12 = chunk.map.part12.Position
					local p13 = chunk.map.part13.CFrame
					local p14 = chunk.map.part14.CFrame
					spawn(function() Utilities.FadeIn(.3) end)
					Utilities.Tween(7, nil, function(a)
						local p = p1:Lerp(p2, a)
						local f = p3:Lerp(p4, a)
					...
					spawn(function() Utilities.FadeIn(.3) end)
					Utilities.Tween(7, nil, function(a)
						local p = p5:Lerp(p6, a)
						local f = p7:Lerp(p8, a)
						cam.CoordinateFrame = CFrame.new(p, f)
					end)
					Utilities.FadeOut(.5)
					wait(.1)
					spawn(function() Utilities.FadeIn(.3) end)
					Utilities.Tween(7, nil, function(a)
						local p = p9:Lerp(p10, a)
						local f = p11:Lerp(p12, a)
						cam.CoordinateFrame = CFrame.new(p, f)
					end)
					Utilities.FadeOut(.5)
					cam.CFrame = p13
					...
					MasterControl:LookAt(telescopedudee.model.HumanoidRootPart.CFrame)
					telescopedudee:LookAt(_p.player.Character.HumanoidRootPart.Position)
					telescopedudee:Say('It looks as if Groudon and Kyogre have dealt their damages and are looking at you to deal the final blow!', 
						'Here\'s your chance! Either catch or stop Rayquaza before it causes more chaos.'
					)
					spawn(function() weathertrio.Rayquaza:Remove() end)
					_p.Battle:doWildBattle(_p.DataManager.currentChunk.regionData.Rayquaza, {battleSceneType = 'Rayquaza'})

					telescopedudee:Say('Well done ' .. _p.PlayerData.trainerName ..  '.', 
						'See you round!'
					)
					Utilities.FadeOut(.4)
					spawn(function() _p.PlayerData:completeEvent('WeatherTrio') end)
					telescopedudee:Remove()
					weathertrio:Remove()
					wait(.2)
					Utilities.FadeIn(.4)
					MasterControl.WalkEnabled = true
		...
		onLoad_chunk66 = function(chunk)
			...
			if completedEvents.FindZElectric then
				pcall(function() chunk.map.FindZElectric['#InanimateInteract']:Remove() end)
				pcall(function() chunk.map.FindZElectric['Main']:Remove() end)
			end
			local stageOrder = {
				[1] = 'Water',
				[2] = 'Grass',
		...
		onLoad_chunk74 = function(chunk)
			...
			Trigger.Touched:connect(function(g)
				...
				_p.RunningShoes:disable()
				MasterControl.WalkEnabled = false
				TweenCamera(cam, 8.7, campart.CFrame)
				...
				chat.bottom = true
				Utilities.FadeOut(.5)
				chat:say("Marshadow has been defeated! Happy Halloween")
		...
		onLoad_chunk13 = function(chunk)
			local CryptDoor = chunk.map.CryptDoor
			local CryptDoor2 = chunk.map.CryptDoor2
			if _p.PlayerData.badges[7] then
			end
		end,
		...
		onLoad_chunk87 = function(chunk)
			...
			if not events.DefeatHoopa then
				...
				touchEvent(nil, chunk.map.HTrigger, false, function()
					...
					IsSpawned = false
					TheBronzeBrick.Main.Attachment.ShineParticles:Remove()
					...
					Utilities.FadeOut(0.1, Color3.new(1, 1, 1))
					TheBronzeBrick.Necklace:Remove()
					wait(1)
					...
					spawn(function()
						...
						if win then
							...
							Jake:LookAt(Vector3.new(-301.673, 1768.14, -5.32))
							spawn(function()
								Jake:Say("[ma]The ancient Rorians bound Hoopa because it Removeed their homes and way of life.")
							end)
							wait(2)
							chat:manualAdvance()
							spawn(function()
							...
							chat:manualAdvance()
							spawn(function()
								Jake:Say("[ma]Cypress planned to leave this world Removeed by Hoopa.")
							end)
							wait(2)
							chat:manualAdvance()
							spawn(function()
							...
							_p.MusicManager:prepareToStack(1)
							bricks:Remove()
							TheBronzeBrick:Remove()
							newChunk.map.PortalGui:Remove()
							...
							newChunk.map.BlackWire:Remove()
							newChunk.map.RedWire:Remove()
							...
							spawn(function()
								...
								newChunk.map.RiggedCrate:Remove()
							end)
							...
							wait(0.9)
							spawn(function()
								BigPortal.PortalFrame:Remove()
							end)
							newChunk.map.BigPortalGui:Remove()
							wait(2)
							...
							spawn(function()
								...
								Tess.model.Head.face.Texture = "rbxassetid://98619596"
								Tess.model.HumanoidRootPart.Anchored = false
								newChunk.map:FindFirstChild("Hoopa"):Remove()
			...
			Dad:Remove()
			Tess:Remove()
			chunk.map.GhostBeam:Remove()
			chunk.map.TheBronzeBrick:Remove()
			chunk.map.RiggedCrate:Remove()
			Tyler:Remove()
			chunk.map.CutsceneMisc:Remove()
			chunk.map.TheBronzeBrick2:Remove()
			chunk.map.hoopapreload:Remove()
			chunk.map.RedWire:Remove()
			chunk.map.BlackWire:Remove()
			chunk.map.BigPortalGui:Remove()
			Looker:Remove()
			rocks:Remove()
		end,
		onLoad_gym8 = function(chunk)        
			...
			chunk.npcs["Pirate Mary"].model.HumanoidRootPart.BodyGyro:Remove()
			chunk.npcs["Pirate Mary"].model.HumanoidRootPart.BodyPosition:Remove()
			chunk.npcs["Pirate Smitty"].model.HumanoidRootPart.BodyGyro:Remove()
			chunk.npcs["Pirate Smitty"].model.HumanoidRootPart.BodyPosition:Remove()
			chunk.npcs["Pirate Peterson"].model.HumanoidRootPart.BodyGyro:Remove()
			chunk.npcs["Pirate Peterson"].model.HumanoidRootPart.BodyPosition:Remove()
			...
			local exitFunc = function()
				...
				Utilities.FadeOut(1)       
				_p.DataManager.currentChunk:Remove()
				local newc = _p.DataManager:loadChunk('chunk76')
				spawn(function()    
					local cam = workspace.CurrentCamera
