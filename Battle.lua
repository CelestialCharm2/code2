return function(_p)
	...
	function BattleClient:doTrainerBattle(data)
		...
		data.kind = 'trainer'
		if not data.musicId then -- Battle Music Trainer
			data.musicId = 17475429160
		end
		...
		if data.vs then
			...
			local ballImage = Utilities.Create 'ImageLabel' {
				BackgroundTransparency = 1.0,
				Image = 'rbxassetid://'..(data.IconId or (data.trainer and data.trainer.IconId) or 13607128001),
	...
	function BattleClient:doVsAnimation(name, imageId, hue, sat, val)
		...
		local v = create 'ImageLabel' {
			BackgroundTransparency = 1.0,
			Image = 'rbxassetid://13607186565', -- VS Decal
	...
	function BattleClient:doPVPBattle(data)
		if self.currentBattle then return end
		local battlethemes = { -- BATTLE MUSIC GOBBLECHANGETHIS
			16823245658, -- Ancient King Battle
			16823237673, -- Rival Battle
			16823297416, -- Victini Battle
			16823273963, -- nECROZMA
			16823271338, -- zINNA THEME
			16823237673, -- PVP BATTLE OST
			16823262747, -- SM Battle Theme
			13068249023, -- XY Battle Theme
		}
		MasterControl.WalkEnabled = false
		MasterControl:Stop()
		MasterControl:Hidden(true)
		...
		Utilities.FadeOut(1)
		if playerListWasEnabled then _p.PlayerList:enable() end
		battle:Remove(true) -- OVH  TODO: use this to change titles; protect the title API (already done)
		cam.FieldOfView = 70
		cam.CoordinateFrame = preBattleCameraCFrame
		if _p.DataManager.currentChunk.indoors then
			_p.DataManager.currentChunk.indoorCamFunc(true)
	...
	function BattleClient:do2v2Battle(data)
		if self.currentBattle then return end
		local battlethemes = {
			131986331851494, -- Default PVP Theme
			10842179046, -- Mummy Battle
			10841926945, -- Rival Battle
			134650945476710, -- Trainer Battle Theme
			10842216951, -- Victini Battle
			16823065306, -- Gym 4 Leader Fight
			16823053775, -- Gym 3 Leader Fight
			10841966565, -- Gym 2 Leader Fight
			10841935520, -- Gym 1 Leader Fight
			72585975743874, -- SM Battle Theme
			90610120383227, -- XY Battle Theme
		}
		MasterControl.WalkEnabled = false
		MasterControl:Stop()
		MasterControl:Hidden(true)
		...
		-- 2v2 TEAM PREVIEW
		_p.MusicManager:stackMusic(74213252048395, 'TeamPreview', .3)
		teamPreview:prepare(battle)
...
--]]
				...
				if a and not a.fainted then
					if a.teamn and a.teamn ~= self.teamn then
						if self.npcPartner then
							...
							local mega = '' --NOT NOTES: preadded for npcs that will mega or zmov (maybe dmax?)
							...
							--NOT NOTES: this is the temp fix for 2v2s that lets client decide npcs move

							choices[i] = 'move '..enabledMoves[math.random(1, #enabledMoves)]..' '..math.random(1,2)..mega--'notmyhalf'
						end
					else
						local moves = a.moves
	...
	function BattleClient:setupScene()
		...
		if self.kind ~= 'pvp' and self.kind ~= '2v2' then
			local musicId = self.musicId or {17475432331, 17475432331} -- old: {424133853, 424134350}, really old: 282237556
